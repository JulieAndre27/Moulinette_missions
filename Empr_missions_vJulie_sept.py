...
gn = geocoders.GeoNames(username='oaumont')  # this API is rate-limited and requires a username linked to an existing account
...
my_data_file_path = "Data/MIS_2022_extrait.xlsx"  # Supported formats are: .xlsx,.xlsm,.xltx,.xltm
...
def rchop(source: str, ending: str) -> str:
    """removes <ending> from the end of <source> if it ends with <ending>"""
    return source[:-len(ending)] if source.endswith(ending) else source


def compute_letter_next_columns(nlist: int, add: int) -> str:
    ...
    # TODO This looks totally useless ??? The only moment we should need letters is to convert the conf file columns to a column id as an integer, which is much simpler...
    n = nlist
    aa = (n + add) // 26
...
for i_rows in range(1, len(col_depart)):  # omit the first line, the column titles
    ...
    if locations_found:
        ...
        if t_type_air in transport_type:
            ...
            if dist_km < d_short_distance_plane:
                emp_co2 = float(dist_km + offset_plane_km) * EF_short_distance_plane
            elif dist_km < d_medium_distance_plane:
                emp_co2 = float(dist_km + offset_plane_km) * EF_medium_distance_plane
            else:  # long-distance plane
                emp_co2 = float(dist_km + offset_plane_km) * EF_long_distance_plane
            ...
            if (location_d.raw['countryCode'] == 'FR') and (location_a.raw['countryCode'] == 'FR'):
                if factor_distance_train * dist_km < d_TER_TGV:
                    ...
                    emp_co2 = factor_distance_train * dist_km * EF_french_TER
                else:
                    emp_co2 = factor_distance_train * dist_km * EF_french_TGV
            elif (location_d.raw['countryCode'] == 'FR') or (location_a.raw['countryCode'] == 'FR'):
                emp_co2 = factor_distance_train * float(dist_km) * EF_half_french_trains
            else:
                emp_co2 = factor_distance_train * float(dist_km) * EF_european_trains

        elif t_type_car.split(',')[0] in transport_type or t_type_car.split(',')[1] in transport_type:
            t_type_main = t_type_car
            emp_co2 = float(dist_km * factor_distance_car) * EF_cars

        else:
            print('Transport type based on distance')
            if dist_km < threshold_unknown_transportation:  # we consider it is train
                t_type_main = t_type_train
        ...
        print(
            f"One-way emissions from {departure} to {arrival} ({round(dist_km)} km) by {t_type_main} is {round(emp_co2, 1)} kg C02e")
