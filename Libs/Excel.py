...
def save_to_file(df_raw: pd.DataFrame, output_path: Path) -> None:
    ...
    with pd.ExcelWriter(str(output_path), engine="xlsxwriter") as writer:
        ...
        sheet_formatted.write(tcomp_vertical_offset, tcomp_column + 3, "Ville départ:")
        sheet_formatted.write(tcomp_vertical_offset, tcomp_column + 4, "Paris")
        sheet_formatted.write(tcomp_vertical_offset, tcomp_column + 5, "Pays départ:")
        sheet_formatted.write(tcomp_vertical_offset, tcomp_column + 6, "FR")
        sheet_formatted.write(tcomp_vertical_offset + 1, tcomp_column + 3, "Ville arrivée:")
        sheet_formatted.write(tcomp_vertical_offset + 1, tcomp_column + 4, "Vienne")
        sheet_formatted.write(tcomp_vertical_offset + 1, tcomp_column + 5, "Pays arrivée:")
        sheet_formatted.write(tcomp_vertical_offset + 1, tcomp_column + 6, "AT")
        tcomp_vertical_offset += 2
        # First column
        transport_format = workbook.add_format({"bg_color": "#ffc000", **f_border_align_center})
        synth_transports = ["Voiture", "Train", "Vol court", "Vol moyen", "Vol long"]
        ...
        total_format = workbook.add_format({"border": 1, "align": "center", "valign": "vcenter"})
        sheet_formatted.write(tcomp_vertical_offset + 1 + len(synth_transports), tcomp_column, "Total", total_format)
        # Second column
        transport_id_format = workbook.add_format({"bg_color": "#acb20c", **f_border_align_center})
        synth_transports_ids = ["ID", "car", "train *", "plane (short)", "plane (med)", "plane (long)"]
        for i, s in enumerate(synth_transports_ids):
        ...
        for j, col in enumerate([tcomp_column_distance, tcomp_column_emission, tcomp_column_uncertainty]):  # other columns
            for i, transp in enumerate(synth_transports):
                sheet_formatted.write_formula(
                    tcomp_vertical_offset + i + 1,
                    tcomp_column + 3 + j,
                    f"SUMIFS({col}:{col}, ${tcomp_column_transport}:${tcomp_column_transport}, ${column_transport_id}{tcomp_vertical_offset + 2 + i}, {tcomp_condition})/1000",
                    synth_cell_format,
                )
        synth_total_format = workbook.add_format({"bg_color": "#ffe699", **f_border_align_center, "num_format": "0.0"})
        for j in range(4):  # totals
            ...
            sheet_formatted.write_formula(
                tcomp_vertical_offset + 6,
                tcomp_column + 2 + j,
                f"SUM({index_to_column(col)}{tcomp_vertical_offset+2}:{index_to_column(col)}{tcomp_vertical_offset+6})",
                synth_total_format
                if j > 0
                else workbook.add_format({"bg_color": "#ffe699", "border": 1, "align": "center", "valign": "vcenter", "num_format": "0"}),
            )
