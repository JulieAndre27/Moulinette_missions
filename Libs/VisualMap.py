...
def _prepare_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["emissions_twoway"] = df["co2e_emissions_kg"] * (1 * (df["round_trip"] == "non") + 1)

    new_df = (
        df.groupby("notparis_address")
        .agg(total_emissions_twoway=("emissions_twoway", "sum"), single_emissions_twoway=("emissions_twoway", "mean"))
        .reset_index()
    )
...
def generate_visual_map(df: pd.DataFrame, output_path: Path) -> None:
    ...
    # kg -> t
    df["single_emissions_twoway"] /= 1e3
    ...
    normalize = Normalize(vmin=df["single_emissions_twoway"].min(), vmax=df["single_emissions_twoway"].max())

    # Plot circles on the map
    df["circle_size"] = df["total_emissions_twoway"] / df["total_emissions_twoway"].max()
    max_radius = 1e2
    for index, row in df.iterrows():
        x, y = m(row["notparis_address"].longitude, row["notparis_address"].latitude)
        m.plot(x, y, "o", markersize=max_radius * row["circle_size"], color=cmap(normalize(row["single_emissions_twoway"])), alpha=0.6)
    ...
    cbar.set_label("Emissions per round trip (tCO2e)")
    ...
    for index, row in df.iterrows():
        ...
        m_europe.plot(x, y, "o", markersize=max_radius * row["circle_size"], color=cmap(normalize(row["single_emissions_twoway"])), alpha=0.6)
